...
public class SumaUno {
	...
	public static void main(String[] args) {
		...
		// podido introducir y comprobamos que la entrada es correcta.
		do {
			...
			if (contador == numeroB.length()) { // Si el contador es igual a la longitud entonces sabemos que todos los
												// caracteres son validos
				verificar = true; // True si los valores son correctos
			}
			if (!verificar) {
				System.out.println("\nNo has introducido un numero hexadecimal");
		...
		for (int i = 0; i < numeroB.length(); i++) {
			...
			if (numeroB.charAt(i) == '9') { // Asi saltamos todos los caracteres que se encuentran entre el 9 y la A
				...
				if (i > 0 && numeroB.charAt(i - 1) != 'F') { // En el caso de que haya mas de un digito a analizar
																// pero no hay acarreo que sumar al siguiente
					numeroFinalB = numeroFinalB.append((char) (numeroB.charAt(i)));
				}
				if (i > 0 && numeroB.charAt(i - 1) == 'F') { // En el caso de que haya mas de un digito a analizar y
																// hay acarreo que sumar al siguiente
					numeroFinalB = numeroFinalB.append((char) (circular + '0'));
				}
			}
			if (i == numeroB.length() - 1 && numeroB.charAt(i) == 'F') { // Si es el ultimo digito a analizar y
																			// conlleva acarreo
